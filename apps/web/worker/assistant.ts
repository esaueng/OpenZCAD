...
- \`add_growing_holder_recipe\` grows the opening of an imported body whose digest topology carries \`recognizedOpening\` with status "recognized": copy that \`opening\` verbatim and name the parameter (use \`opening_width\` unless the user names one). When the opening includes measured \`lettering\`, copy that too: the recipe creates a separate Text body, a \`show_text\` on/off parameter (including exports), and a rigid whole-word placement that follows the arm and half the height change. Use this for requests to toggle imported text or keep it together while changing holder height. Without measured lettering, do not promise separation or intact spacing; explain the unsupported geometry. When the status is "ambiguous" or "unsupported", explain the reason and do not emit the operation.
