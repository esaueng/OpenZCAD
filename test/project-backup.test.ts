...
it('marks foreign archived sources local throughout a copied project so account sync rearchives them', async () => {
  const backup = fixture();
  const source = Object.values(backup.document.nodes).find(
    (node) =>
  ...
  if (
    ...
    throw new Error('Fixture missing STEP');
  source.data.artifactId = toArtifactId('artifact_foreign');
  backup.document.commandLog.push({
    kind: 'import-step',
    payload: { artifactId: 'artifact_foreign' },
    label: 'Import',
    replayVersion: 1,
    timestamp: new Date().toISOString()
  });
  backup.files.push(
    await packBackupFile(
      {
        artifactId: toArtifactId('artifact_foreign'),
        projectId: backup.document.projectId,
        kind: 'step-import',
        name: 'part.step',
        objectKey: 'old-account',
        contentType: 'model/step',
        createdAt: new Date().toISOString(),
        metadata: {}
      },
  ...
  const copy = importProjectCopy(backup, toUserId('new'));
  const imported = copy.document.nodes[source.id];
  if (
    imported?.kind !== 'feature' ||
    imported.data.featureKind !== 'imported-step'
  )
    throw new Error('Copy missing STEP');
  expect(imported.data.artifactId).toMatch(/^artifact_local_/);
  expect(copy.files[0]?.artifact.artifactId).toBe(imported.data.artifactId);
  expect(copy.document.commandLog.at(-1)?.payload).toEqual({
