...
describe('cloudflare adapters', () => {
  ...
  it('rejects a finalize that would pass the account artifact storage ceiling', async () => {
    const owner = toUserId('user_quota_owner');
    const projectId = toProjectId('project_quota');
    const artifactId = toArtifactId('artifact_quota');
    const uploadSessionId = toUploadSessionId('upload_quota');
        ...
        first: async () => {
          ...
          if (query.includes('SELECT artifact_id, object_key')) {
            return {
              ...
              object_key: 'step_export/uploads/part.step',
          ...
          if (query.includes('COALESCE(SUM(artifacts.bytes)')) {
            return { total: 900 };
          }
          return null;
        },
    ...
    const service = new D1R2PersistenceService({
      ...
      ARTIFACT_ACCOUNT_STORAGE_LIMIT_BYTES: '1000'
    });
    ...
    // Nothing is written: the session stays valid so freeing space and
    // retrying the finalize succeeds without a re-upload.
    expect(batch).not.toHaveBeenCalled();

    // The same finalize passes when it fits under the ceiling.
    const roomier = new D1R2PersistenceService({
      DB: { prepare, batch } as unknown as D1Database,
      ARTIFACTS: {
        head: vi.fn().mockResolvedValue({ size: 200 })
      } as unknown as R2Bucket,
      ARTIFACT_ACCOUNT_STORAGE_LIMIT_BYTES: '2000'
    });
    await expect(
      roomier.finalizeArtifact(owner, {
        projectId,
        uploadSessionId,
        artifactId
      })
    ).resolves.toMatchObject({ artifactId, bytes: 200 });
  });
