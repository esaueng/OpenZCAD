...
| Exact geometry          | Both adapters implement mirror-copy and shell; BrepKit and proved convex-planar OCCT solids implement sharp positive-outward solid offset                                                                                                              | Working safe subset              | Curved, non-convex, or unproved OCCT offset fails closed; BrepKit mirror also refuses dense blended/boolean bodies when the pinned kernel does not preserve measured solid volume |
| STEP                    | Exact OCCT import/export plus the existing validated direct-edit subset                                                                                                                                                                                | Partial                          | Six-family imported-feature recognition is read-only and isolated until a live exact adjacency adapter and coordinated commands ship                                              |
...
| Face-attached sketches  | Schema-v5 face references resolve an exact planar face at the sketch’s history position on both exact adapters; a deterministic frame is rebuilt from that face                                                                                        | Working for current references   | Deleted, ambiguous, non-planar, and unsupported faces stop rebuild. Legacy attachments without lineage use the stored migration frame with a warning                              |
| Multi-profile extrusion | Multiple exact closed profiles on BrepKit and OCCT; deterministic AI contract available                                                                                                                                                                | Working                          | Keep cross-kernel seam and history-position fixtures green as topology contracts evolve                                                                                           |
| Modeling UI             | Mirror, shell opening-face selection, and solid-offset forms share command preflight and expression validation                                                                                                                                         | Working safe subset              | The UI exposes the same OCCT sharp-offset limitation as the kernel; output lineage remains hash-only                                                                              |
| Direct edits            | Planar offsets and cylindrical hole/boss radius edits run on both kernels with analytic and volume validation                                                                                                                                          | Partial                          | Coordinated counterbore/countersink, pocket-depth, and taper-angle edits remain disabled                                                                                          |
...
| Kernel delivery         | Exact adapter/BrepKit and OCCT load lazily in the worker; jobs have tagged lifecycle state and stale-result gating                                                                                                                                     | Working                          | First-load latency remains hardware-dependent and must be measured separately from UI bundle startup                                                                              |
...
- The exact adapter/BrepKit boundary is now dynamic, OCCT remains lazy, and
  canonical derived results use the bounded worker-local cache described in
