...
| Modeling UI             | Mirror, shell opening-face selection, and solid-offset forms share command preflight and expression validation                                                                                                                                         | Working safe subset              | Output lineage remains hash-only. Solid offset is no longer withheld on imported documents — that gate described OpenCascade, which no longer builds them                                                                   |
...
- The exact adapter/BrepKit boundary is now dynamic, OCCT no longer ships at
  all, and canonical derived results use the bounded worker-local cache
  described in [ADR-015](adrs/ADR-015-bounded-exact-rebuild-cache.md).
