...
| Direct edits            | Planar offsets, cylindrical hole/boss radius edits, through-hole resize, and feature removal run on both kernels with analytic and volume validation                                                                                                   | Partial                                 | Coordinated counterbore/countersink, pocket-depth, and taper-angle edits remain disabled; on Remus, hole closing refuses where the boolean falls back to a mesh and defeature refuses any body that is not all-planar                         |
...
| Kernel delivery         | The exact adapter and Remus load lazily in the worker; jobs have tagged lifecycle state and stale-result gating                                                                                                                                        | Working                                 | First-load latency remains hardware-dependent and must be measured separately from UI bundle startup                                                                                                                                          |
