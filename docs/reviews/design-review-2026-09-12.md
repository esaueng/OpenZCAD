...
- **Recommendation.** Negate both the direction vector and the distance when `totalDistance < 0` at the two call sites, keeping lineage carriers from the signed span. Run the union's strict validation (or at least the unified-face acceptance) on add/cut results. Give the union refusal the `Details` disclosure the tool card already has.
...
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- | ----------------------------- |
| 0 · Trust                | C1 extrude sign + validated add/cut · C3 validated commits everywhere · C4 text-field undo · C6 translate warnings, drop ids · C7 suppression not a warning · C8 enablement on committed state · I7 outcome copy                            | Very high — stops silent corruption and lies | 6–9                           |
