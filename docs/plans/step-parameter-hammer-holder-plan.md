...
| Phase 7: acceptance                    | #300; browser synthetic import-to-export plus Vitest backup/STEP/STL coverage; 2026-09-17 [acceptance matrix](../qa/2026-09-17/holder-acceptance-matrix.md) with unit-scaled recognition, pinned ambiguity and reload-interruption cases | Inch documents refuse the recipe at the kernel's bridge union (K07); the live walkthrough remains.              |
...
- **Phase 6 (PR 7, 2026-09-11):** the preview stands in for its own result
  body only (a document with more parts keeps them; before, the viewer showed
  the preview alone), the status reads "Parameter preview", and an edit that
  also changes a non-control parameter (the bore diameter, or anything else)
  stays exact-only. Measured on the hammer in the browser (M-series Mac, dev
  build, `oz:viewer.bodies:begin` mark after the field's change event):
  preview presented in 32–144 ms for width and height edits; exact result
  0.7–3.5 s warm. Before this PR the exact result took 31–77 s: the two hole
  direct edits carried the whole parameter scope in their digest, so any
  width edit invalidated the prefix at the first hole edit and re-measured
  every source reference, each with a full opening recognition. Direct edits
