...
| **L — lineage brittleness.** A hash-only `offset-face` must re-measure the same area on every replay. 11 of 17 lineage classes are hash-only (`topology-lineage.ts:82`; booleans became carrier-derived on 2026-09-02), so any face downstream of a pattern, shell, chamfer, or direct edit — or a boolean face whose carrier is shared or split — breaks on the next upstream edit. Nothing warns before the drag. | `exact-direct-edit-ops.ts:1080` | "no longer matches its recorded measurements", or the edit silently drops out of replay (`test/direct-edit-face-repair.test.ts`) |
