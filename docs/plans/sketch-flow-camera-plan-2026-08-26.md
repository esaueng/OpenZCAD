...
| 4.2 | Sketch-mode chrome crossfades with the entry glide: the sketch rail and palette were the only mode chrome with no entrance, so they snapped in on the first frame while the camera flew for 800 ms. Both now fade over `--dur-base`, matching the tool card's existing `tool-card-in` precedent. Enter-only, like that card — the exit would need delayed unmount past the React cleanup, the same thing the audit deliberately left undone for the gizmo | **done** |
