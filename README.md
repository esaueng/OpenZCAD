...
- Imported meshes — STL, 3MF, OBJ, glTF binary (`.glb`) and PLY — build on the exact kernel through its own importers, sewn into a shell so they can be mirrored, shelled, and offset. Every format lands in the same `imported-mesh` feature, so the document, autosave and exports know only one mesh path. It stays a mesh body: no parametric reconstruction is attempted, and a boolean against an exact body is refused by name rather than approximated.
