...
- Imported meshes — STL, 3MF, OBJ, glTF binary (`.glb`) and PLY — build on the exact kernel through its own importers, sewn into a shell so they can be mirrored, shelled, and offset. Every format lands in the same `imported-mesh` feature, so the document, autosave and exports know only one mesh path. One import is one body, several shells included: a file holding several objects is sewn into one body carrying all of them, and a file whose triangles the rebuild cannot take is refused by name at import — with the kernel's own reason — rather than importing behind a success message and leaving a feature with no body. A 3MF's declared length unit is read from the package and converted to millimetres, and its `<build>` section is honoured: per-item transforms are applied, an object placed twice imports twice, and an object the build never places is left out. The other mesh formats declare no unit and follow STL's millimetre convention. It stays a mesh body: no parametric reconstruction is attempted, and a boolean against an exact body is refused by name rather than approximated.
