...
- Imported meshes — STL, 3MF, OBJ, glTF binary (`.glb`) and PLY — build on the exact kernel through its own importers, sewn into a shell so they can be mirrored, shelled, and offset. Every format lands in the same `imported-mesh` feature, so the document, autosave and exports know only one mesh path. One import is one body: a file holding several objects is refused by name, because separate shells cannot be sewn into the single shell the feature rebuilds. A 3MF's declared length unit is read from the package and converted to millimetres; the other mesh formats declare none and follow STL's millimetre convention. It stays a mesh body: no parametric reconstruction is attempted, and a boolean against an exact body is refused by name rather than approximated.
