...
- onboard the `auth.esau.app` and `zcad.esau.app` sending domains and keep the `EMAIL` binding restricted to `login@auth.esau.app` for sign-in and `noreply@zcad.esau.app` for project invitations;
...
- set `AUTH_MODE=email-code`, `ENVIRONMENT=beta`, `AUTH_EMAIL_FROM=login@auth.esau.app`, and `PROJECT_INVITATION_EMAIL_FROM=noreply@zcad.esau.app` (the checked-in beta config also sets `PRODUCTION_GUARD`, which makes the worker refuse development auth outright);
